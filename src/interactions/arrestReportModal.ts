...
export async function handleArrestReportModal(interaction: ModalSubmitInteraction): Promise<void> {
  ...
  try {
    ...
    logger.error("handleArrestReportModal failed", error, { userId: interaction.user.id });
    await interaction.editReply({
